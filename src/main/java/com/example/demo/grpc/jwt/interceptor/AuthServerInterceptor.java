...
import com.example.demo.grpc.jwt.exception.AuthException;
import com.example.demo.grpc.jwt.exception.UnauthenticatedException;
...
public class AuthServerInterceptor implements ServerInterceptor {
	...
	Logger LOGGER = Logger.getLogger(AuthServerInterceptor.class.getName());
...
LOGGER.info("Calling method " + methodName);
    ...
    private <ReqT> void validateAllowedAnnotation(ReqT request, JwtContextData contextData, String methodName) {
    	LOGGER.info("In validateAllowedAnnotation");
    ...
    private <ReqT> void authorizeOwnerOrRoles(ReqT request, JwtContextData contextData, AllowedMethod allowedMethod) {
    	LOGGER.info("In authorizeOwnerOrRoles");
        if(contextData == null) throw new AuthException("Missing JWT data.");
